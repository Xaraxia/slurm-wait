...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("slurm_report.log"), logging.StreamHandler()],
)
...
def plot_average_wait_times(jobs_df, output_dir):
    ...
    hourly_wait_times = (
        hardware_jobs.set_index("datetime")
        .resample("1H")
        .agg({"wait_time": "median"})
        .reset_index()
    )
    ...
    plt.title(
        "Median Wait Time for Hardware-Limited Jobs", fontsize=14, fontweight="bold"
    )
...
def plot_wait_time_distribution(jobs_df, output_dir):
    ...
    plt.title(
        "Distribution of Wait Times for Hardware-Limited Jobs",
        fontsize=14,
        fontweight="bold",
    )
...
def plot_wait_time_by_partition(jobs_df, output_dir):
    ...
    partition_stats = (
        hardware_jobs.groupby("partition")
        .agg(
    ...
    plt.bar(
        positions, partition_stats["avg_wait"], width=0.6, alpha=0.7, color="steelblue"
    )

    # Add job count as text
    for i, (avg, count) in enumerate(
        zip(partition_stats["avg_wait"], partition_stats["job_count"])
    ):
        plt.text(
            i, avg + avg * 0.05, f"{count} jobs", ha="center", va="bottom", fontsize=9
        )

    # Configure axes
    plt.title(
        "Median Wait Time by Partition (Hardware Limited Jobs Only)",
        fontsize=14,
        fontweight="bold",
    )
...
def plot_resource_utilization(stats_df, output_dir):
    ...
    stats_df["cpu_utilization"] = (
        stats_df["allocated_cpus"] / stats_df["total_cpus"] * 100
    )
    stats_df["memory_utilization"] = (
        stats_df["allocated_memory"] / stats_df["total_memory"] * 100
    )
    ...
    # CPU utilization
    ax1.plot(
        stats_df["datetime"], stats_df["cpu_utilization"], linewidth=2, color="#1f77b4"
    )
...
def plot_correlation_matrix(jobs_df, stats_df, output_dir):
    ...
    hourly_jobs = (
        hardware_jobs.set_index("datetime")
        .resample("1H")
        .agg({"wait_time": "median", "job_id": "count"})
        .reset_index()
    )
    ...
    plt.title(
        "Correlation Between Cluster Metrics and Wait Times",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def plot_wait_time_vs_utilization(jobs_df, stats_df, output_dir):
    ...
    if len(merged_df) < 2:
        logger.warning(
            "Not enough valid data points to plot wait time vs. CPU utilization"
        )
        plt.figure(figsize=(10, 8))
        plt.title(
            "Median Wait Time vs. CPU Utilization\n(Insufficient data for trend analysis)",
            fontsize=14,
            fontweight="bold",
        )
    ...
    try:
        ...
        if (
            merged_df["cpu_utilization"].std() > 0.001
        ):  # Ensure some variation in x values
            z = np.polyfit(
                merged_df["cpu_utilization"], merged_df["wait_time"] / 3600, 1
            )
            p = np.poly1d(z)
            x_range = np.linspace(
                merged_df["cpu_utilization"].min(),
                merged_df["cpu_utilization"].max(),
                100,
            )
...
def generate_summary_stats(jobs_df, stats_df):
    ...
    if not hardware_jobs.empty:
        ...
        # Wait time by user
        user_stats = (
            hardware_jobs.groupby("user")
            .agg(avg_wait=("wait_time", "median"), job_count=("job_id", "count"))
...
def generate_html_report(jobs_df, stats_df, summary, output_dir, start_date, end_date):
    ...
    <body>
        ...
        <div class="summary-box">
            ...
            <div class="stat-row">
                ...
                <div class="stat-item">
                    <div class="stat-value">{max_wait_hours:.2f} hours</div>
                    <div class="stat-label">Maximum Wait Time</div>
                </div>
            </div>
        ...
        <div class="plot-container">
            ...
            <p>Comparison of average wait times across different partitions, highlighting which \
                partitions experience the longest delays.</p>
        </div>
    ...
    if summary["avg_cpu_utilization"] > high_util_threshold:
        recommendations.append(
            "Consider expanding cluster capacity as average CPU utilization exceeds 75%."
        )

    if summary["high_util_percentage"] > 30:
        recommendations.append(
            f"The cluster is operating above 90% capacity for {summary['high_util_percentage']:.1f}% \
                of the time, suggesting hardware resources are consistently stretched."
        )

    if avg_wait_hours > long_wait_threshold:
        recommendations.append(
            f"Median wait times are high at {avg_wait_hours:.2f} hours, indicating insufficient resources to meet demand."
        )
    ...
    for part_data in summary["partition_wait_times"]:
        ...
        if wait_hours > long_wait_threshold and part_data["job_count"] > 10:
            partition_recommendations.append(
                f"Partition '{part_data['partition']}' has particularly high wait times \
                    ({wait_hours:.2f} hours) across {part_data['job_count']} jobs."
            )
    ...
    if not recommendations and not partition_recommendations:
        html_content += "<p>Based on the current data, the cluster appears to be sufficiently sized for \
            the workload. Wait times are within acceptable limits and resource utilization is balanced.</p>"
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="report", help="Output directory for reports"
    )
    ...
    try:
        ...
        logger.info(
            f"Loaded {len(jobs_df)} jobs and {len(stats_df)} cluster stat entries"
        )
        ...
        logger.info("Report is available at: %s", report_path)
