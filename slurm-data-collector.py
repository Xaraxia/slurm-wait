...
------------------------------
Collects data about job wait times due to insufficient hardware rather than policy restrictions.
Stores results in a SQLite database for later analysis.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("slurm_collector.log"),
...
# Hardware limitation reason codes - jobs with these reasons are waiting due to insufficient hardware
...
def get_pending_jobs():
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"First job structure: {json.dumps(jobs_list[0] if jobs_list else {})}") 
            ...
            for job in jobs_list:
                ...
                if isinstance(submit_time, dict) and 'number' in submit_time:
                    ...
                    logger.warning(f"Unexpected submit_time structure for job {job_id}: {submit_time}")
                    submit_time = 0
                elif not isinstance(submit_time, (int, float)):
                    logger.warning(f"Invalid submit_time format for job {job_id}: {submit_time}")
                    submit_time = 0                
                ...
                if 'tres_per_job' in job:
                    ...
                    for part in parts:
                        if '=' in part:
                            ...
                            elif key == 'mem' and any(value.endswith(unit) for unit in ['K', 'M', 'G', 'T']):
                ...
                jobs.append({
                    ...
                    'submission_time': datetime.fromtimestamp(submit_time).isoformat() if submit_time else '',
        ...
        if "unknown option" in str(e) and "--json" in str(e):
            logger.error("The --json option is not supported in this version of Slurm. Please update Slurm or use the text-based version.")
...
def get_cluster_stats():
    ...
    try:
        ...
        available_nodes = sum(1 for node in nodes.values() if 'idle' in node['state'] or 'mix' in node['state'])
    ...
    try:
        ...
        for job in pending_jobs:
            ...
            INSERT INTO pending_jobs (
                timestamp, job_id, partition, job_name, user, wait_time, 
                requested_nodes, requested_cpus, requested_mem, reason, is_hardware_limited, tres_per_job
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                timestamp,
                job['job_id'],
        ...
        conn.commit()
        logger.info(f"Data stored successfully: {len(pending_jobs)} jobs and cluster stats")
    except Exception as e:
        conn.rollback()
...
def main():
    ...
    parser.add_argument('--max-runtime', type=int, default=0, help='Maximum runtime in seconds (0 = run forever)')
    args = parser.parse_args()
    
    logger.info(f"Starting Slurm wait time data collector")
    ...
    try:
        while True:
            ...
            if args.max_runtime > 0 and (time.time() - start_time) >= args.max_runtime:
                logger.info(f"Maximum runtime reached, exiting")
                break
